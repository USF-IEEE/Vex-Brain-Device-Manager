...
class DeviceManager { 
    ...
    void add_device(TerriBullDevices_DeviceHeader* device);
    TerriBullDevices_DeviceHeader* get_device(uint32_t port);
    bool processMessage(const uint8_t* buffer, size_t size);

    private:
    std::map<uint32_t, TerriBullDevices_DeviceHeader*> devices;
